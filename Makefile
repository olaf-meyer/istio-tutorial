...
build-customer:
	mvn clean package -f customer/java/quarkus/ -DskipTests -Pnative -Dquarkus.native.container-build=true -Dquarkus.native.container-runtime=podman -Dquarkus.native.additional-build-args=-H:ReflectionConfigurationFiles=reflection-config.json
...
build-recommendation:
	mvn clean package -f recommendation/java/quarkus/ -DskipTests -Pnative -Dquarkus.native.container-build=true -Dquarkus.native.container-runtime=podman -Dquarkus.native.additional-build-args=-H:ReflectionConfigurationFiles=reflection-config.json
	docker build -t quay.io/omeyer/istio-tutorial-recommendation:v3.0-kafka -f recommendation/java/quarkus/src/main/docker/Dockerfile.native recommendation/java/quarkus/
